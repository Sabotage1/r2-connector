...
describe("ReaPrimeApi", () => {
  ...
  it("wraps WebUI skin update and GitHub release install APIs", async () => {
    vi.spyOn(globalThis, "fetch").mockImplementation((input, init = {}) => {
      ...
      if (method === "PUT" && url.pathname === "/api/v1/webui/skins/default") {
        return Promise.resolve(new Response(JSON.stringify({ success: true, skinId: "workflow-skin" }), { status: 200 }));
      }
      if (method === "POST" && url.pathname === "/api/v1/webui/skins/update") {
        return Promise.resolve(new Response(JSON.stringify({ message: "Skin update check completed" }), { status: 200 }));
      }
      if (method === "POST" && url.pathname === "/api/v1/webui/skins/install/github-release") {
        return Promise.resolve(new Response(JSON.stringify({ success: true, repo: "roy/workflow-skin" }), { status: 200 }));
      }
      return Promise.reject(new Error(`Unhandled ${method} ${url.pathname}`));
    });
    const api = new ReaPrimeApi("http://machine:8080");
    ...
    await expect(api.setDefaultWebUISkin("workflow-skin")).resolves.toEqual({ success: true, skinId: "workflow-skin" });
    await expect(api.updateWebUISkins()).resolves.toEqual({ message: "Skin update check completed" });
    await expect(api.installSkinFromGithubRelease({ repo: "roy/workflow-skin", asset: "workflow-skin.zip", prerelease: false })).resolves.toEqual({
      success: true,
      repo: "roy/workflow-skin"
    });

    expect(fetch).toHaveBeenCalledWith("http://machine:8080/api/v1/webui/skins", expect.objectContaining({ method: "GET" }));
    expect(fetch).toHaveBeenCalledWith("http://machine:8080/api/v1/webui/skins/default", expect.objectContaining({ method: "GET" }));
    expect(fetch).toHaveBeenCalledWith(
      "http://machine:8080/api/v1/webui/skins/default",
      expect.objectContaining({ method: "PUT", body: JSON.stringify({ skinId: "workflow-skin" }) })
    );
    expect(fetch).toHaveBeenCalledWith("http://machine:8080/api/v1/webui/skins/update", expect.objectContaining({ method: "POST" }));
    expect(fetch).toHaveBeenCalledWith(
      "http://machine:8080/api/v1/webui/skins/install/github-release",
      expect.objectContaining({ method: "POST", body: JSON.stringify({ repo: "roy/workflow-skin", asset: "workflow-skin.zip", prerelease: false }) })
    );
  });
});
